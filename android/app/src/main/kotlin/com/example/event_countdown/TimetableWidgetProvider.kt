    ...
    companion object {
        ...
        fun updateWidgetDirectly(
            ...
            try {
                ...
                android.util.Log.e("TimetableWidget", "Update failed for widget $widgetId", e)
                try {
                    val fallbackViews = RemoteViews(context.packageName, R.layout.timetable_widget_layout)
                    fallbackViews.setTextViewText(R.id.timetable_widget_day, "Timetable")
                    fallbackViews.setTextViewText(R.id.timetable_widget_date, "Tap to open")
                    fallbackViews.setViewVisibility(R.id.timetable_widget_empty, View.VISIBLE)
                    for (rowId in ROW_IDS) {
                        fallbackViews.setViewVisibility(rowId, View.GONE)
                    }
                    appWidgetManager.updateAppWidget(widgetId, fallbackViews)
                } catch (e2: Exception) {
                    android.util.Log.e("TimetableWidget", "Fallback also failed", e2)
                }
            }
        }
