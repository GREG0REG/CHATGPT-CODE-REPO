...
/// Result of a unified restore via [ExportImportService.restoreFromFile].
class ImportRestoreResult {
  final bool success;
  final String? exportType;
  final int? itemCount;
  final String? errorMessage;

  const ImportRestoreResult({
    required this.success,
    this.exportType,
    this.itemCount,
    this.errorMessage,
...
class ExportImportService {
  ...
  static Future<Map<String, dynamic>> _readZipFile(String filePath) async {
    ...
    // IMPORTANT: `data.json` is NOT always a JSON object. Events-only exports
    // (exportType == 'events') store a plain JSON array of events, while full
    // exports (exportType == 'full') store a JSON object with a 'tables' key.
    // Force-casting to Map<String, dynamic> here used to throw:
    //   "type 'List<dynamic>' is not a subtype of type 'Map<String, dynamic>'"
    // for every events-only restore. Decode dynamically instead and let
    // callers branch on the runtime type.
    final dynamic dataObject = jsonDecode(dataString);
  ...
  static Future<ImportPreview> previewImportFile(String filePath) async {
    ...
    try {
      ...
      // `decoded` intentionally stays `dynamic`: an events-only export is a
      // JSON List, a full export (or legacy full-table backup) is a JSON Map.
      dynamic decoded;
      String? exportType;
      String? appVersionRaw;
      String? exportedAtRaw;
      int? expVersion;
      String? checksum;
      ...
      if (_isZipFile(filePath)) {
        ...
        appVersionRaw = manifest?['appVersion'] as String?;
        exportedAtRaw = manifest?['exportedAt'] as String?;
        expVersion = manifest?['exportVersion'] as int?;
        checksum = manifest?['checksum'] as String?;
        decoded = zipData['data'];
        // _readZipFile() already recomputes the checksum from the raw bytes
        // it read out of the archive and throws if it doesn't match the
        // manifest, so reaching this point means the file's integrity is
        // already confirmed — no need (and no safe way) to redo that check
        // against the re-encoded Dart object here.
        ...
        if (decoded is Map<String, dynamic>) {
          ...
          appVersionRaw = decoded['appVersion'] as String?;
          exportedAtRaw = decoded['exportedAt'] as String?;
          expVersion = decoded['exportVersion'] as int?;
          exportType ??= decoded.containsKey('events') ? 'events' : null;
        } else if (decoded is List) {
          // Plain-JSON events export (see exportEventsAsPlainJson) has no
          // wrapper object at all — it's just the array of events.
          exportType = 'events';
        }
      }

      final appVersion = _safeString(appVersionRaw, 'unknown');
      final exportedAt = _safeDateTimeParse(exportedAtRaw);
      ...
      if (decoded is List) {
        ...
        if (decoded['tables'] is Map) {
          final dataMap = decoded['tables'] as Map;
          for (final entry in dataMap.entries) {
            final rows = entry.value;
            if (rows is List) {
              tableRowCounts[entry.key.toString()] = rows.length;
      ...
      if (checksum != null && !checksumValid) {
  ...
  /// Restores from any supported backup file (`.ecbackup` or `.json`,
  /// events-only or full-data), auto-detecting the correct import routine.
  ///
  /// This is the recommended entry point for "Restore backup" UI flows —
  /// e.g. the first-launch restore prompt — since it removes the need for
  /// the caller to guess whether a given file is an events-only export or a
  /// full-data export before deciding which import method to call.
  static Future<ImportRestoreResult> restoreFromFile(String filePath) async {
    final preview = await previewImportFile(filePath);

    if (preview.exportType == 'error') {
      return ImportRestoreResult(
        success: false,
        errorMessage: preview.warnings.isNotEmpty
    ...
    try {
      if (preview.exportType == 'full') {
        await importAllData(filePath);
        return ImportRestoreResult(
          success: true,
          exportType: 'full',
          itemCount: preview.totalRows,
        );
      } else if (preview.exportType == 'events') {
        final count = await importFromJson(filePath);
        return ImportRestoreResult(
          success: true,
          exportType: 'events',
          itemCount: count,
      ...
      return ImportRestoreResult(
        success: false,
        errorMessage: 'Unrecognized backup type: ${preview.exportType}',
      );
    } catch (e) {
      return ImportRestoreResult(success: false, errorMessage: '$e');
    }
  }
