...
/// ============================================
/// VIRTUAL EVENT EXPANSION SERVICE
///
/// Core principle: LAZY expansion. Only compute next 2-3 occurrences
/// per recurring event. No background tasks. Called at query time.
/// ============================================
class RecurrenceService {
  RecurrenceService._();

  /// Maximum future occurrences to generate per recurring event.
  static const int _maxOccurrencesPerEvent = 3;

  /// Lookahead window: don't generate beyond this many days ahead.
  static const int _maxLookaheadDays = 730; // 2 years

  /// ============================================
  /// MAIN ENTRY: Expand all recurring events into virtual instances
  /// Returns a flat list sorted by primarySortMillis, nearest first.
  /// ============================================
  ...
  /// ============================================
  /// Get ONLY the single next upcoming occurrence
  /// Ultra-fast path for widget refresh.
  /// ============================================
  ...
  static List<Event> _generateVirtualOccurrences(
    ...
    switch (event.recurrence) {
      ...
      case RecurrenceType.yearly:
        if (event.yearlyUseSpecificDates &&
            event.yearlySpecificDates.isNotEmpty) {
  ...
  static List<Event> _generateYearlySpecificOccurrences({
    ...
    if (virtuals.length < maxCount) {
      ...
      final candidatesNextYear = specificDates.map((sd) {
        final dt = sd.toDateTime(nextYear);
        return _OccurrenceCandidate(dt, sd);
      }).toList();
  ...
  static Event _buildVirtualEvent(
    ...
    return Event(
      id: -event.id!, // Negative ID = virtual instance
      title: event.title,
