...
class _AddEditEventScreenState extends State<AddEditEventScreen> {
  ...
  Future<void> _deleteReminder(CustomReminder reminder) async {
    if (reminder.id != null) await DatabaseHelper.instance.deleteCustomReminder(reminder.id!);
      ...
      builder: (ctx) => _DateTimeEditDialog(
        ...
        startTime: existing?.customStartTimeMillis != null
            ? TimeOfDay.fromDateTime(
                DateTime.fromMillisecondsSinceEpoch(existing!.customStartTimeMillis!))
            : widget.baseStartTime,
        deadlineTime: existing?.customDeadlineMillis != null
            ? TimeOfDay.fromDateTime(
                DateTime.fromMillisecondsSinceEpoch(existing!.customDeadlineMillis!))
      ...
      setState(() {
        _dateDetails[key] = YearlySpecificDate(
          ...
          customStartTimeMillis: result.startTime != null
              ? DateTime(date.year, date.month, date.day, result.startTime!.hour,
                      result.startTime!.minute)
                  .millisecondsSinceEpoch
              : null,
          customDeadlineMillis: result.deadlineTime != null
              ? DateTime(date.year, date.month, date.day, result.deadlineTime!.hour,
                      result.deadlineTime!.minute)
                  .millisecondsSinceEpoch
              : null,
        );
      });
              ...
              child: GridView.builder(
                ...
                itemBuilder: (ctx, index) {
                  ...
                  final isSelected = _selectedDates
                      .any((d) => d.month == date.month && d.day == date.day);
                  final hasCustomTimes = _dateDetails['${date.month}-${date.day}']
                          ?.customStartTimeMillis != null ||
                      _dateDetails['${date.month}-${date.day}']
...
// ============================================
// DATE TIME EDIT DIALOG (per specific date)
// ============================================
      ...
      actions: [
        ...
        TextButton(
          onPressed: () => Navigator.pop(
            context,
            _DateTimeOverrides(startTime: _startTime, deadlineTime: _deadlineTime),
          ),
...
// ============================================
// REMINDER DIALOG (Session 4 - unchanged)
// ============================================
