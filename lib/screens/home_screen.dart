...
class _HomeScreenState extends State<HomeScreen> with WidgetsBindingObserver {
  ...
  // ── Daily study minutes computed from events ──
  int _getTodayStudyMinutes() {
    ...
    int total = 0;
    for (final e in _events) {
      if (e.dateMillis >= todayStart && e.dateMillis < todayEnd) {
        if (e.startTimeMillis != null && e.deadlineMillis != null) {
          total += (e.deadlineMillis! - e.startTimeMillis!) ~/ 60000;
        }
      }
    }
    return total;
  ...
  // ── Subject time breakdown for today ──
  Map<String, int> _getSubjectTimeToday() {
    final now = DateTime.now();
    final todayStart = DateTime(now.year, now.month, now.day).millisecondsSinceEpoch;
    final todayEnd = todayStart + const Duration(days: 1).inMilliseconds;

    final times = <String, int>{'Physics': 0, 'Chemistry': 0, 'Biology': 0};
    for (final e in _events) {
      if (e.dateMillis >= todayStart && e.dateMillis < todayEnd) {
        if (e.startTimeMillis != null && e.deadlineMillis != null) {
          final mins = (e.deadlineMillis! - e.startTimeMillis!) ~/ 60000;
          final tag = e.subjectTag ?? '';
          if (tag.toLowerCase().contains('physics')) times['Physics'] = times['Physics']! + mins;
          else if (tag.toLowerCase().contains('chemistry')) times['Chemistry'] = times['Chemistry']! + mins;
          else if (tag.toLowerCase().contains('biology')) times['Biology'] = times['Biology']! + mins;
        }
      }
    }
    return times;
  ...
  // ── 7-day streak data ──
  List<_DayStreak> _getWeekStreak() {
    final now = DateTime.now();
    ...
    for (int i = 6; i >= 0; i--) {
      ...
      final hasEvents = _events.any((e) => e.dateMillis >= dayStart && e.dateMillis < dayEnd);
  ...
  Widget build(BuildContext context) {
    ...
    final todayMins = _getTodayStudyMinutes();
    final subjectTimes = _getSubjectTimeToday();
    final weekStreak = _getWeekStreak();
