...
class BackupService {
  ...
  static const String _kAppName = 'StudyFlow';

  // Filenames this app has used for backups over time. `findRecentBackup()`
  // must recognize all of them, since a file written by one export path
  // (e.g. ExportImportService's 'studyflow_export_...') must still be
  // discoverable by the restore flow that uses this class.
  static const List<String> _backupFilePrefixes = [
    'studyflow_full_export_',
    'studyflow_export_',
    'studyflow_events_',
    'event_countdown_backup_', // legacy naming, kept for upgrading users
  ];

  static bool _looksLikeBackupFile(File f) {
    final ext = p.extension(f.path).toLowerCase();
    if (ext != '.ecbackup' && ext != '.json') return false;
    final name = p.basename(f.path);
    return _backupFilePrefixes.any((prefix) => name.startsWith(prefix));
  }
  ...
  static Future<String?> findRecentBackup() async {
    try {
      ...
      if (downloadsDir.existsSync()) {
        candidates.addAll(
          downloadsDir.listSync().whereType<File>().where(_looksLikeBackupFile),
        );
      }
      ...
      if (docsDir.existsSync()) {
        candidates.addAll(
          docsDir.listSync().whereType<File>().where(_looksLikeBackupFile),
        );
      }

      if (candidates.isEmpty) return null;

      // Prefer checksum-verified .ecbackup files over legacy plain .json,
      // then pick whichever candidate was actually modified most recently
      // (sorting by path string previously broke once filenames mixed
      // different timestamp formats/lengths).
      candidates.sort((a, b) {
        final aIsEc = p.extension(a.path).toLowerCase() == '.ecbackup';
        final bIsEc = p.extension(b.path).toLowerCase() == '.ecbackup';
        if (aIsEc != bIsEc) return aIsEc ? -1 : 1;
