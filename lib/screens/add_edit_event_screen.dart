...
class _AddEditEventScreenState extends State<AddEditEventScreen> {
  ...
  // NEET fields — using model enums directly
  ...
  StudyDuration _studyDuration = StudyDuration.medium;
  final Set<String> _selectedStudyModeTags = {};
  ...
  void initState() {
    ...
    if (_isEditing) {
      ...
      _neetExamType = e.neetExamTypeEnum;
      _revisionRound = e.revisionRoundEnum;
      _isPyqSession = e.isPyqSession;
      _isCompleted = e.isCompleted;
      // NEW: Load working study features
      _difficulty = e.difficultyEnum;
      _studyDuration = e.studyDurationEnum;
      _selectedStudyModeTags.addAll(e.studyModeTags);
    } else {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final targetScore = _targetScoreController.text.trim().isEmpty
          ? null
      ...
      final event = Event(
        ...
        // NEET fields — now properly wired
        isNeetExam: _neetExamType != NeetExamType.none,
        neetTotalMarks: _neetExamType != NeetExamType.none ? 720 : null,
        neetTargetScore: targetScore,
        neetSubjectFocus: _subjectController.text.trim().isEmpty ? null : _subjectController.text.trim(),
        targetScore: targetScore,
        neetExamType: _neetExamType.dbValue,
        revisionRound: _revisionRound.dbValue,
        isPyqSession: _isPyqSession,
        // NEW: Working study features
        difficulty: _difficulty.dbValue,
        studyDuration: _studyDuration.dbValue,
        studyModeTagsJson: _selectedStudyModeTags.isNotEmpty
            ? jsonEncode(_selectedStudyModeTags.toList())
            : null,
      );
  ...
  // ─── NEET Exam Type Dropdown ─────────────────────────────────────
  Widget _buildNeetExamTypeDropdown() {
    final items = NeetExamType.values.map((type) =>
    ...
    return Column(
      ...
      children: [
        const Padding(
          padding: EdgeInsets.fromLTRB(0, 8, 0, 4),
          child: Text('NEET Exam Type', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
        ),
        DropdownButtonFormField<NeetExamType>(
          value: _neetExamType,
          ...
          items: items,
          onChanged: (v) => setState(() => _neetExamType = v ?? NeetExamType.none),
        ),
      ],
    );
  }

  // ─── Target Score (shown only for Mock Test) ─────────────────────
  Widget _buildTargetScoreField() {
    if (_neetExamType != NeetExamType.mockTest) return const SizedBox.shrink();
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        ...
        TextFormField(
          ...
          decoration: const InputDecoration(
            ...
            helperText: 'Set a realistic target for this mock test',
          ),
          validator: (v) {
            if (v == null || v.trim().isEmpty) return null;
            final score = int.tryParse(v.trim());
            if (score == null) return 'Enter a valid number';
            if (score < 0 || score > 720) return 'Score must be between 0 and 720';
            return null;
          },
        ),
      ],
  ...
  // ─── Subject Marks Weight Info Chip ──────────────────────────────
  Widget _buildSubjectMarksWeight() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const SizedBox(height: 16),
        const Padding(
          ...
          child: Text('Subject Marks Weight', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
        ),
        Wrap(
          spacing: 8,
          runSpacing: 8,
          children: [
            _buildWeightChip('Physics', '180', Colors.blue),
            _buildWeightChip('Chemistry', '180', Colors.green),
            _buildWeightChip('Biology', '360', Colors.orange),
          ],
        ),
      ],
    );
  ...
  Widget _buildWeightChip(String subject, String marks, Color color) {
    return Chip(
      avatar: CircleAvatar(
        backgroundColor: color.withOpacity(0.2),
        child: Text(subject[0], style: TextStyle(color: color, fontSize: 12, fontWeight: FontWeight.bold)),
      ),
  ...
  // ─── PYQ Session Toggle ──────────────────────────────────────────
  Widget _buildPyqToggle() {
    return SwitchListTile(
      contentPadding: EdgeInsets.zero,
      title: const Text('Previous Year Questions'),
      subtitle: const Text('Mark this as a PYQ practice session'),
      value: _isPyqSession,
      onChanged: (v) => setState(() => _isPyqSession = v),
    );
  }

  // ─── Revision Round Dropdown ─────────────────────────────────────
  Widget _buildRevisionRoundDropdown() {
    final items = RevisionRound.values.map((round) =>
      DropdownMenuItem(value: round, child: Text(round.label)),
    ).toList();
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Padding(
          padding: EdgeInsets.fromLTRB(0, 8, 0, 4),
          child: Text('Revision Round', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
        ),
        DropdownButtonFormField<RevisionRound>(
          value: _revisionRound,
          decoration: const InputDecoration(
            border: OutlineInputBorder(),
            prefixIcon: Icon(Icons.repeat_outlined),
          ),
          items: items,
          onChanged: (v) => setState(() => _revisionRound = v ?? RevisionRound.round1),
        ),
      ],
    );
  }

  // ─── Study Timer Estimate (NOW WORKING) ──────────────────────────
  Widget _buildStudyTimerEstimate() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const SizedBox(height: 16),
        const Padding(
          padding: EdgeInsets.fromLTRB(0, 0, 0, 4),
          child: Text('Estimated Study Duration', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
        ),
        Card(
          elevation: 0,
          ...
          child: Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: StudyDuration.values.map((duration) {
                final isSelected = _studyDuration == duration;
                return InkWell(
                  onTap: () => setState(() => _studyDuration = duration),
                  borderRadius: BorderRadius.circular(8),
                  child: Container(
                    padding: const EdgeInsets.symmetric(vertical: 8, horizontal: 4),
                    decoration: BoxDecoration(
                      color: isSelected ? duration.color.withOpacity(0.15) : null,
                      borderRadius: BorderRadius.circular(8),
                      border: isSelected ? Border.all(color: duration.color, width: 1.5) : null,
                    ),
                    child: Row(
                      children: [
                        Icon(duration.icon, size: 20, color: duration.color),
                        const SizedBox(width: 12),
                        Expanded(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text(duration.label, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13)),
                              Text(duration.subtitle, style: TextStyle(fontSize: 12, color: Theme.of(context).colorScheme.outline)),
                            ],
                          ),
                        ),
                        if (isSelected)
                          Icon(Icons.check_circle, color: duration.color, size: 20),
                      ],
                    ),
                  ),
                );
  ...
  // ─── Difficulty Level Selector (NOW WORKING) ─────────────────────
  Widget _buildDifficultySelector() {
    return Column(
      ...
      children: [
        ...
        const Padding(
          padding: EdgeInsets.fromLTRB(0, 0, 0, 4),
          child: Text('Difficulty Level', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
        ),
        SegmentedButton<StudyDifficulty>(
          segments: StudyDifficulty.values.map((d) =>
            ButtonSegment(value: d, label: Text(d.label)),
          ).toList(),
          selected: {_difficulty},
          onSelectionChanged: (selected) {
            if (selected.isNotEmpty) setState(() => _difficulty = selected.first);
          },
        ),
      ],
    );
  ...
  // ─── Study Mode Tags (NOW WORKING) ─────────────────────────────
  Widget _buildStudyModeTags() {
    final tags = [
      'Concept Building',
      'Formula Revision',
      'Diagram Practice',
    ...
    return Column(
      ...
      children: [
        ...
        const Padding(
          padding: EdgeInsets.fromLTRB(0, 0, 0, 4),
          child: Text('Study Mode Tags', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
        ),
        Wrap(
          spacing: 8,
          runSpacing: 8,
          children: tags.map((tag) {
            final isSelected = _selectedStudyModeTags.contains(tag);
            return FilterChip(
              label: Text(tag),
              selected: isSelected,
              onSelected: (selected) {
                setState(() {
                  if (selected) {
                    _selectedStudyModeTags.add(tag);
                  } else {
                    _selectedStudyModeTags.remove(tag);
                  }
                });
              },
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            // ─── NEW NEET FEATURES ──────────────────────────────────
            const SizedBox(height: 16),
            _buildNeetExamTypeDropdown(),
            _buildTargetScoreField(),
            _buildSubjectMarksWeight(),
            const SizedBox(height: 8),
            _buildPyqToggle(),
            const SizedBox(height: 8),
            _buildRevisionRoundDropdown(),
            _buildStudyTimerEstimate(),
