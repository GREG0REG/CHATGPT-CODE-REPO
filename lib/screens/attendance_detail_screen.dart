...
class _AttendanceDetailScreenState extends State<AttendanceDetailScreen>
  ...
  String _semesterProjection(double percentage, int total, double required) {
    ...
    final semesterStart = _subject['semesterStartMillis'] != null
        ? DateTime.fromMillisecondsSinceEpoch(
            (_subject['semesterStartMillis'] as int?) ?? 0)
        : null;
    final semesterEnd = _subject['semesterEndMillis'] != null
        ? DateTime.fromMillisecondsSinceEpoch(
            (_subject['semesterEndMillis'] as int?) ?? 0)
        : null;
  ...
  Widget _buildStatusChips(ColorScheme cs) {
    final present = (_subject['present'] as int?) ?? 0;
    final absent = (_subject['absent'] as int?) ?? 0;
    final late = (_subject['late'] as int?) ?? 0;
    final excused = (_subject['excused'] as int?) ?? 0;
        ...
        children: [
          ..._schedules.map((sched) {
            ...
            final dayName = dayIndex >= 0 && dayIndex < 7
                ? _dayName((sched['dayOfWeek'] as int?) ?? 1)
                : '?';
      ...
      child: Column(
        children: recentLogs.map((log) {
          ...
          return Dismissible(
            ...
            onDismissed: (_) => _deleteLog((log['id'] as int?) ?? 0),
            child: Card(
