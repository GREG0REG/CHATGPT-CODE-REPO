  ...
  Future<void> _markComplete(int topicId) async {
    try {
      ...
      if (topic != null) {
        ...
        await DatabaseHelper.instance.generateRevisionSchedules(topicId);
      }
      await _loadData();
    } catch (e) {
  ...
  Widget _buildDeadlineList(List<Map<String, dynamic>> deadlines, ColorScheme cs, {required bool isOverdue}) {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              isOverdue
                  ? 'you are on top of all your deadlines'
                  : 'add chapter deadlines to track upcoming work',
    ...
    return ListView.builder(
      ...
      itemBuilder: (ctx, index) {
        ...
        final topicName = d['topicName'] as String? ?? 'Unknown Topic';
        final subjectName = d['subjectName'] as String? ?? 'Unknown Subject';
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    ...
                    if (isOverdue)
                      Container(
                        ...
                        child: const Text('overdue', style: TextStyle(color: Colors.red, fontSize: 11, fontWeight: FontWeight.w600)),
                      ),
                  ],
                ),
                const SizedBox(height: 8),
                ...
                Row(
                  children: [
                    Icon(Icons.calendar_today, size: 14, color: isOverdue ? Colors.red : cs.onSurfaceVariant),
                    const SizedBox(width: 4),
                    Text(
                      '${targetDate.day}/${targetDate.month}/${targetDate.year}',
                      style: TextStyle(color: isOverdue ? Colors.red : cs.onSurfaceVariant, fontSize: 13),
                    ),
                    if (!isOverdue) ...[
