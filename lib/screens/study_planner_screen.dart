...
class _StudyPlannerScreenState extends State<StudyPlannerScreen>
  ...
  Future<void> _loadData() async {
    ...
    // Find active plan (most recently created active one, or first active)
    StudyPlan? active;
          ...
          child: Column(
            ...
            children: [
              ...
              if (_activePlan != null) ...[
                ...
                // Stats grid
                _buildStatsGrid(cs, daysLeft, remaining, totalItems, paceStatus, paceColor),
                const SizedBox(height: 20),

                // Subject breakdown if available
                if (subject != null)
                  _buildSubjectBreakdown(cs, subject),
              ] else ...[
                _buildNoPlanCard(cs),
              ],
  ...
  Widget _buildStatsGrid(ColorScheme cs, int daysLeft, int remaining, int totalItems, String paceStatus, Color paceColor) {
    return Column(
      children: [
        Row(
          children: [
            _buildStatCard('days left', '$daysLeft', 'until exam', cs,
                accentColor: daysLeft < 7 ? Colors.red : null),
            const SizedBox(width: 12),
        ...
        Row(
          children: [
            _buildStatCard('buffer days', '${_activePlan?.bufferDays ?? 7}', 'reserved for revision', cs,
                accentColor: Colors.green),
            const SizedBox(width: 12),
            _buildStatCard('daily target', '${_activePlan?.dailyStudyMinutes ?? 120}m', 'per day', cs,
            ...
            child: Row(
              children: [
                Container(
                  width: 10,
                  height: 10,
                  decoration: BoxDecoration(color: paceColor, shape: BoxShape.circle),
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: Column(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              'create a plan to organize your neet prep and track your progress',
              style: TextStyle(color: cs.onSurfaceVariant),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        '${dayItems.where((i) => i.completed).length}/${dayItems.length} done',
              ...
              child: Form(
                ...
                child: ListView(
                  children: [
                    ...
                    const SizedBox(height: 4),
                    Text(
                      'generate a smart schedule based on your actual syllabus',
                      ...
                      children: [
                        ...
                        Expanded(
                          child: Slider(
                            ...
                            max: 30,
                            divisions: 30,
                            value: _bufferDays.toDouble(),
                            onChanged: (v) => setState(() => _bufferDays = v.round()),
                          ),
                        ),
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
