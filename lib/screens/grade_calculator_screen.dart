...
// COMPLETE REPLACEMENT — NEET Timetable v7.0 (FULLY FIXED)
// FIXED: Time text NEVER pops out of cards — all text properly constrained
// FIXED: Conflict cards render side-by-side cleanly with proper overflow handling
// FIXED: Drag handles work consistently with proper gesture zones
// FIXED: Tasks render in unified timeline alongside classes
// FIXED: All cards use Clip behavior to prevent any overflow
// FIXED: Minimum content heights enforced, auto-hide secondary text when cramped
// FIXED: Time text uses compact format, never exceeds card bounds
// FIXED: Proper padding calculations so nothing bleeds outside
// NEW: All features accessible from timetable screen (no invisible features)
// NEW: Visual polish — softer shadows, better spacing, consistent borders
...
class _TimetableScreenState extends State<TimetableScreen> {
  ...
  /// Groups overlapping items so they render side-by-side
  ...
  // ============================================
  // DRAG HANDLING (FIXED — both handles use RawGestureDetector)
  // ============================================
  void _onDragStart(int id, bool isTop, int currentStart, int currentEnd, bool isTask) {
    ...
    setState(() {
      _draggingId = id;
      _draggingTop = isTop;
      _dragOriginalMinutes = isTop ? currentStart : currentEnd;
  ...
  void _onDragUpdate(DragUpdateDetails details, int id, bool isTop, int originalStart, int originalEnd) {
    ...
    setState(() {
      if (isTop) {
        _dragCurrentStart = (originalStart + snappedDelta).clamp(_timelineStartMinutes, originalEnd - 30);
      } else {
        _dragCurrentEnd = (originalEnd + snappedDelta).clamp(originalStart + 30, _timelineEndMinutes);
      }
    });
  }
  ...
  // ============================================
  // TIMELINE (FULLY FIXED — no text overflow ever)
  // ============================================
  ...
  List<Widget> _buildConflictGroupRow(List<Map<String, dynamic>> group, List<Map<String, dynamic>> allConflicts, ColorScheme cs) {
    ...
    if (group.length == 1) {
      ...
      final height = math.max(52.0, rawHeight);
    ...
    // Multiple items — render side by side in a Row with proper constraints
    final firstItem = group.first;
    final groupStart = (firstItem['startTimeMinutes'] as int?) ?? 0;
    final top = _minutesToPixels(groupStart);

    // Find the max end time in the group for container height
    int maxEnd = groupStart;
    for (final item in group) {
      final itemStart = (item['startTimeMinutes'] as int?) ?? 0;
      final itemEnd = (item['endTimeMinutes'] as int?) ?? (itemStart + 60);
      if (itemEnd > maxEnd) maxEnd = itemEnd;
    }
    final groupHeight = math.max(52.0, _durationToPixels(maxEnd - groupStart));
      ...
      Positioned(
        ...
        child: Row(
          ...
          children: group.map((item) {
            ...
            final itemStart = (item['startTimeMinutes'] as int?) ?? 0;
            final itemEnd = (item['endTimeMinutes'] as int?) ?? (itemStart + 60);
            final itemHeight = math.max(52.0, _durationToPixels(itemEnd - itemStart));
            ...
            return Expanded(
              child: Padding(
                ...
                child: ClipRRect(
                  borderRadius: BorderRadius.circular(10),
                  child: isTask
                      ? _buildTaskCard(item, color, cs, true, isDragging, itemHeight)
                      : _buildClassCard(item, color, cs, true, isDragging, itemHeight, itemStart, itemEnd),
                ),
              ),
            );
          }).toList(),
  ...
  // ============================================
  // CLASS CARD (FULLY FIXED — no text ever pops out)
  // ============================================
  Widget _buildClassCard(
    ...
    // Height budget: drag handles (14px each, tighter) + content area
    const double dragHandleHeight = 14.0;
    final double contentHeight = height - (dragHandleHeight * 2);
    
    // Determine what we can show based on available height
    final bool showType = contentHeight > 28;
    final bool showRoom = contentHeight > 42 && room.isNotEmpty;
    final bool showPomo = contentHeight > 38 && _isNeetSubject(subject);
    final bool showTime = contentHeight > 22;

    return Container(
      height: height,
      decoration: BoxDecoration(
        color: color.withOpacity(isDragging ? 0.25 : 0.15),
        borderRadius: BorderRadius.circular(10),
        border: Border.all(
      ...
      child: ClipRRect(
        ...
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            // TOP DRAG HANDLE (FIXED: RawGestureDetector, smaller)
            RawGestureDetector(
              ...
              gestures: {
                VerticalDragGestureRecognizer: GestureRecognizerFactoryWithHandlers<VerticalDragGestureRecognizer>(
                  ...
                  (instance) {
                    instance.onStart = (_) => _onDragStart(c['id'] as int, true, originalStart, originalEnd, false);
                    instance.onUpdate = (d) => _onDragUpdate(d, c['id'] as int, true, originalStart, originalEnd);
                    instance.onEnd = (_) => _onDragEnd(c['id'] as int, false);
                  },
                ),
              },
              child: Container(
            ...
            // CARD BODY (tap to edit) — FIXED: Constrained height, no overflow
            Expanded(
              child: InkWell(
                onTap: () => _editClass(c),
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      // Subject name — always show, ellipsis if too long
                      Row(
                        children: [
                      ...
                      // Spacer
                      const Spacer(),
                      // Footer row: time + optional room + optional pomo
                      if (showTime)
                        Row(
                              ...
                              InkWell(
                                ...
                                child: Container(
                                  ...
                                  child: Row(
                                    mainAxisSize: MainAxisSize.min,
                                    children: [
                                      Icon(Icons.play_arrow, size: 8, color: Colors.deepPurple.shade600),
                        ...
                        // BOTTOM DRAG HANDLE (FIXED: RawGestureDetector like top)
            RawGestureDetector(
              behavior: HitTestBehavior.translucent,
              gestures: {
                VerticalDragGestureRecognizer: GestureRecognizerFactoryWithHandlers<VerticalDragGestureRecognizer>(
                  () => VerticalDragGestureRecognizer(),
                  (instance) {
                    instance.onStart = (_) => _onDragStart(c['id'] as int, false, originalStart, originalEnd, false);
                    instance.onUpdate = (d) => _onDragUpdate(d, c['id'] as int, false, originalStart, originalEnd);
                    instance.onEnd = (_) => _onDragEnd(c['id'] as int, false);
                  },
                ),
              },
  ...
  // ============================================
  // TASK CARD (FULLY FIXED — no text ever pops out)
  // ============================================
  Widget _buildTaskCard(
    ...
    final bool showPomo = height > 40 && isStudyBlock;
        ...
        child: InkWell(
          ...
          child: Padding(
            ...
            child: Column(
              ...
              mainAxisSize: MainAxisSize.min,
              children: [
                // Title row — always show
                Row(
                  children: [
                    Icon(_typeIcon(type), size: 10, color: isCompleted ? Colors.grey : color),
                    const SizedBox(width: 3),
                    Expanded(
                      child: Text(
                        title,
